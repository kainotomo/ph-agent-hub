...
class TestDemoE2E:
    ...
    async def test_set_and_get_demo_tenant(self, e2e_db_session):
        ...
        async with AsyncSessionLocal() as db:
            ...
            db.add(demo_tenant)
            await db.flush()
            await set_demo_tenant(db, demo_tenant.id)
            await db.commit()

        async with AsyncSessionLocal() as db:
            tenant = await get_demo_tenant(db)
            assert tenant is not None
            assert tenant.id == demo_tenant.id
            assert tenant.is_demo is True

    async def test_demo_jwt_creation(self, e2e_db_session):
        """Test 3: Create a demo JWT and verify claims."""
        from src.core.jwt import create_demo_token, decode_guest_token
        from src.core.config import settings

        tenant_id = str(uuid.uuid4())
        token = create_demo_token(tenant_id=tenant_id)
    ...
    async def test_demo_token_ttl(self, e2e_db_session):
        """Test 4: Demo token has a reasonable TTL."""
        from src.core.jwt import create_demo_token
        from src.core.config import settings

        tenant_id = str(uuid.uuid4())
        token = create_demo_token(tenant_id=tenant_id, ttl_minutes=60)
        assert token is not None

    async def test_demo_context_validation(self, e2e_db_session):
        ...
        token = create_demo_token(tenant_id=tenant_id)
        payload = decode_guest_token(token)
        ctx = DemoContext(tenant_id=payload["sub"], token=token)
        assert ctx.tenant_id == tenant_id
        assert ctx.token == token

    async def test_guest_token_rejected_for_admin(self, e2e_db_session):
